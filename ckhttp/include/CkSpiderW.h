...
class CkSpiderW : public CkSpider
{
	...
	void getUrlDomainW(const wchar_t *w1,CkString &domainOut)
		{
		CkString sTemp1; sTemp1.appendU(w1);
		this->GetUrlDomain(sTemp1.getUtf8(),domainOut);
		}
	...
	const wchar_t *getUrlDomainW(const wchar_t *w1)
		{
		unsigned long idx = nextIdxW(); m_resultStringW[idx].clear();
		CkString sTemp1; sTemp1.appendU(w1);
		this->GetUrlDomain(sTemp1.getUtf8(),m_resultStringW[idx]);
		return m_resultStringW[idx].getUnicode();
		}
