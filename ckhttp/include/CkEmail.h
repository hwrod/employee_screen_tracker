...
class CkEmail : public CkObject
{
	...
	bool AddRelatedData(const char *nameInHtml, const unsigned char *data, unsigned long dataLen, CkString &outStrContentId){return AddRelatedData(nameInHtml,data,dataLen,&outStrContentId);}
	...
	*/
	...
	void GenerateFilename(CkString &outStrFilename);
	...
	void GetMime(CkString &outStrMime);
	// Returns the email converted to XML.
	void GetXml(CkString &outStrXml);
	...
	const char *addRelatedData(const char *nameInHtml, const unsigned char *data, unsigned long dataLen);
