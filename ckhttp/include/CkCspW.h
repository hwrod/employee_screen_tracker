...
class CkCSPW : public CkCSP
{
	...
	const wchar_t *getHashAlgorithmNameW(long index)
		{
		unsigned long idx = nextIdxW(); m_resultStringW[idx].clear();
		
		this->GetHashAlgorithmName(index,m_resultStringW[idx]);
		return m_resultStringW[idx].getUnicode();
		}
	...
	const wchar_t *keyContainerW(void)
		{
		unsigned long idx = nextIdxW(); m_resultStringW[idx].clear();
		this->get_KeyContainer(m_resultStringW[idx]);
		return m_resultStringW[idx].getUnicode();
		}
