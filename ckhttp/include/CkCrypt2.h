...
class CkCrypt2  : public CkObject
{
	...
	void GetEncodedKey(const char *encoding, CkString &outKey);
	void GetEncodedIV(const char *encoding, CkString &outIV);

	void SetSecretKey(const unsigned char *keyData, int numBytes);
	void SetIV(const unsigned char *ivData, int numBytes);

	void Encode(const CkByteData &bData, const char *encoding, CkString &outStr);
	...
	void HmacBytesENC(const CkByteData &inBytes, CkString &outEncodedHmac);
	void HmacStringENC(const char *inText, CkString &outEncodedHmac);
